class <- read.table("class.txt",header = TRUE,sep = "\t",row.names = 1)
...
png("class_anova.png",width = 7600,height = 6000,res = 600)
ggplot(class,aes(x = Group,y = value,color = Group)) +
    ...
    ylab("The relative abundance of dominant classes among different origins") +
    theme_bw()+
    theme(axis.ticks.length = unit(0.4,"lines"), 
          axis.ticks = element_line(color='black'),
          axis.line = element_line(colour = "black"), 
          axis.title.x=element_blank(),
          axis.title.y=element_text(colour='black', size=18,face = "bold",vjust = 3),
